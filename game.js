...
class RetroAudio {
    ...
    playDeal() {
        ...
        const gainNode = this.ctx.createGain(); gainNode.gain.value = 0.5;
...
function showMessage(msg, duration = 0, isWin = false, isLose = false) {
    gameMessage.innerText = msg;
    gameMessage.classList.remove('hidden');
    gameMessage.classList.remove('text-green-400', 'text-red-400', 'scale-110');
    if (isWin) {
        gameMessage.classList.add('text-green-400', 'scale-110');
    } else if (isLose) {
...
function checkDealerBlackjack() {
    if (dealerHand.hasBlackjack) {
        ...
        if (playerHands[0].hasBlackjack) {
            ...
            setTimeout(() => resolveGame("Dealer Blackjack. You lose.", false, true), 2000);
        }
    } else {
        if (insuranceBet > 0) {
        ...
        if (playerHands[0].hasBlackjack) {
            ...
            setTimeout(() => resolveGame("Blackjack! You win 3:2!", true, false), 1500);
        } else {
...
function resolveGame(forcedMessage = null, isWin = false, isLose = false) {
    ...
    updateUI(false);
    if (forcedMessage) { showMessage(forcedMessage, 0, isWin, isLose); } else { showMessage(resultMsg, 0, hasWon, hasLost); }
...
async function fetchAIReview(gameData) {
    ...
    /*
       ...
       "You are a Blackjack coach. Analyze the play based on basic strategy and the True Count. Provide a brief, easy-to-understand review focusing on the optimal play. Do NOT use overly complex jargon like "True Count" without explaining it simply. The response should be concise, visually organized, and have a clear takeaway. Output your review based on the language preference indicated. Ensure there are translations for English, Chinese (Simplified), and Thai."
    */

    const promptText = `
You are a Blackjack coach. Analyze the play based on basic strategy and card counting. Give a very brief, simple review without complex math jargon. Tell the player the optimal play and why. If they made a mistake, explain the risk simply. Output in: ${document.getElementById("languageSelect") ? document.getElementById("languageSelect").value : "English"}.

Game Data:
- Dealer Upcard: ${gameData.dealerUpcard}
- Player Initial Cards: ${gameData.playerInitialCards.join(', ')}
- True Count: ${gameData.trueCount}
- Player Actions: ${gameData.actions.join(' -> ') || 'None'}
    `;
